...
(function() {
    ...
    _.templateSettings = {
        evaluate    : /<%([\s\S]+?)%>/g,
        interpolate : /<%=([\s\S]+?)%>/g,
        //interpolate : /\{\{(.+?)\}\}/g, // changes template delimeters to {{ }} mustache.js style
        escape      : /<%-([\s\S]+?)%>/g
    };
...
/**
 * Underscore Templating Class
 *
 * @dependencies - underscore.js
 * @dependencies - underscore-mixins.js
...
window.Template.prototype.preload = function() {
    let self = this;
    if(self._render === null && !self.isFetching) {
        ...
        _.fetchTemplate(
            ...
            function(renderMethod) {
                ...
                for(let i = 0, l = self.onReady.length; i < l; i++) {
